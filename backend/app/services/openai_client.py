...
def build_generation_prompt(request: JournalGenerationRequest) -> str:
    ...
    return (
        ...
        "如果用户补充信息包含 templateId，请优先使用对应模板 variant：quiet_story 留白独白，hero_memory 主照片日记，timeline_trip 时间线小旅行，pocket_grid 口袋页，ticket_day 票根备忘，magazine_note 杂志留白，before_after 前后对照，moodboard_stack 情绪堆叠，recipe_memo 餐桌配方，letter_page 写给今天，chapter_scroll 长卷章节，field_notes 观察手记，split_scene 双场景切换，detail_index 细节索引，map_journey 路线地图，weekly_spread 周记分栏，day_dashboard 日程看板，scrapbook_story 剪贴故事。"
        "模板不是单纯照片摆法，而是叙事结构：chapter_scroll 要按开头、转场、结尾写；field_notes 要写观察到的细节；split_scene 要区分两个场景或状态；detail_index 要用一张主图带出几个小细节；map_journey 要讲起点、停靠点和终点；weekly_spread 要按连续几天或几个片段分栏；day_dashboard 要保留清单、完成感和小结；scrapbook_story 要用主图、素材碎片和短句讲回忆。"
        f"{template_guide}"
