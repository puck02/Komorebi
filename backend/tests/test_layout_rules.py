...
def test_layout_rules_allows_story_order_preserved_in_offset_collage_coordinates():
    payload = layout_payload(image_ids=["img_1", "img_2"])
    payload["content"]["sections"] = [
        {"id": "section_1", "title": "第一段", "imageIds": ["img_1", "img_2"], "body": "第一段正文。", "mood": []}
    ]
    payload["layout"]["sections"] = [
        {
            "sectionId": "section_1",
            ...
            "height": 920,
            "images": [
                {"imageId": "img_1", "x": 92, "y": 360, "width": 420, "height": 320, "rotation": -4},
                {"imageId": "img_2", "x": 424, "y": 260, "width": 360, "height": 270, "rotation": 4},
            ],
    ...
    assert not has_issue(issues, "sectionImageMatch", "high", "版式章节图片与内容章节不一致")
...
def test_layout_rules_allow_sparse_captions_for_dense_story_variants():
    payload = layout_payload(image_ids=["img_1", "img_2"])
    payload["content"]["captions"] = [
        {"imageId": "img_1", "text": "主图旁注"},
        {"imageId": "img_2", "text": "收在正文里"},
    ]
    payload["content"]["sections"] = [
        {"id": "section_1", "title": "两张照片", "imageIds": ["img_1", "img_2"], "body": "咖啡喝完后，走到路口。", "mood": []}
    ]
...
def test_layout_rules_report_caption_far_from_matching_photo():
    payload = layout_payload(image_ids=["img_1", "img_2"])
    payload["content"]["captions"] = [
        {"imageId": "img_1", "text": "窗边咖啡"},
        {"imageId": "img_2", "text": "路口灯光"},
    ...
    payload["layout"]["sections"] = [
        {
            ...
            "texts": [
                {"role": "body", "x": 112, "y": 700, "width": 820, "fontSize": 32},
                {"role": "caption", "x": 596, "y": 602, "width": 360, "fontSize": 22},
                {"role": "caption", "x": 120, "y": 542, "width": 360, "fontSize": 22},
            ],
            "decorations": [],
        }
...
def test_layout_rules_report_section_body_too_long_for_human_journal():
    payload = layout_payload()
    long_body = "这是一段本来应该拆成几句短记的正文，" * 8
    payload["content"]["sections"] = [
        {"id": "section_1", "title": "窗边", "imageIds": ["img_1"], "body": long_body, "mood": []}
    ]
    payload["layout"]["sections"] = [
        {
            "sectionId": "section_1",
    ...
    assert has_issue(issues, "sectionCopyAlignment", "medium", "章节正文过长，手帐记录应拆成短句")
